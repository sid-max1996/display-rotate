...
bool display_rotate::rotateByDisplayPosition(int orientation, int x, int y) {
    bool result = false;
    DISPLAY_DEVICE d;
    DEVMODE dm;
    d.cb = sizeof(DISPLAY_DEVICE);
    ...
    int index = 0;
    while (true) {
        if (!EnumDisplayDevices(NULL, index, &d, EDD_GET_DEVICE_INTERFACE_NAME)) {
            break;
        }
        index += 1;
...
Napi::Boolean display_rotate::rotateByDisplayPositionWrapped(const Napi::CallbackInfo& info) 
{
  Napi::Env env = info.Env();

  if (info.Length() < 3) {
    Napi::TypeError::New(env, "Two params expected orientation, x, y").ThrowAsJavaScriptException();
  }

  if (!info[0].IsNumber() || !info[1].IsNumber() || !info[2].IsNumber()) {
    Napi::TypeError::New(env, "orientation or x or y not a number").ThrowAsJavaScriptException();
  } 
  ...
  int orientation = first.Int32Value();
  int x = second.Int32Value();
  int y = third.Int32Value();
  ...
  bool returnValue = display_rotate::rotateByDisplayPosition(orientationParam, x, y);
...
Napi::Object display_rotate::Init(Napi::Env env, Napi::Object exports) 
{
  ...
  exports.Set(
    "rotateByDisplayPosition", Napi::Function::New(env, display_rotate::rotateByDisplayPositionWrapped)
  );
